<?php

namespace Database\Factories;

use App\Models\Category;
use App\Models\Lead;
use App\Models\LeadItem;
use Illuminate\Database\Eloquent\Factories\Factory;

/**
 * @extends \Illuminate\Database\Eloquent\Factories\Factory<\App\Models\LeadItem>
 */
class LeadItemFactory extends Factory
{
    protected $model = LeadItem::class;

    public function definition(): array
    {
        $price = fake()->randomFloat(2, 20, 300);

        return [
            'lead_id' => Lead::factory(),
            'category_id' => null,
            'product_id' => null,
            'sku' => fake()->optional()->bothify('LEAD-????-####'),
            'title' => fake()->words(3, true),
            'description' => fake()->optional()->sentence(),
            'quantity' => 1,
            'price' => $price,
            'buy_price' => $price * fake()->randomFloat(2, 0.4, 0.7),
            'dwt' => null,
            'precious_metal' => null,
            'condition' => fake()->randomElement([
                LeadItem::CONDITION_NEW,
                LeadItem::CONDITION_LIKE_NEW,
                LeadItem::CONDITION_USED,
            ]),
            'is_added_to_inventory' => false,
            'date_added_to_inventory' => null,
        ];
    }

    public function preciousMetal(?string $type = null): static
    {
        return $this->state(fn (array $attributes) => [
            'precious_metal' => $type ?? fake()->randomElement([
                LeadItem::METAL_GOLD_10K,
                LeadItem::METAL_GOLD_14K,
                LeadItem::METAL_GOLD_18K,
                LeadItem::METAL_SILVER,
                LeadItem::METAL_PLATINUM,
            ]),
            'dwt' => fake()->randomFloat(4, 0.5, 10),
        ]);
    }

    public function gold10k(): static
    {
        return $this->preciousMetal(LeadItem::METAL_GOLD_10K);
    }

    public function gold14k(): static
    {
        return $this->preciousMetal(LeadItem::METAL_GOLD_14K);
    }

    public function gold18k(): static
    {
        return $this->preciousMetal(LeadItem::METAL_GOLD_18K);
    }

    public function silver(): static
    {
        return $this->preciousMetal(LeadItem::METAL_SILVER);
    }

    public function platinum(): static
    {
        return $this->preciousMetal(LeadItem::METAL_PLATINUM);
    }

    public function withCategory(): static
    {
        return $this->state(fn (array $attributes) => [
            'category_id' => Category::factory(),
        ]);
    }

    public function damaged(): static
    {
        return $this->state(fn (array $attributes) => [
            'condition' => LeadItem::CONDITION_DAMAGED,
        ]);
    }
}
