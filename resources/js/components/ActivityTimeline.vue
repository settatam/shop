...
<template>
    <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
        <h2 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">{{ title }}</h2>

        <!-- Loading skeleton -->
        <div v-if="isLoading" class="space-y-4">
            <div class="animate-pulse">
                <div class="mb-2 h-4 w-16 rounded bg-gray-200 dark:bg-gray-700"></div>
                <div class="space-y-3">
                    <div v-for="i in 3" :key="i" class="flex items-start gap-3">
                        <div class="size-8 shrink-0 rounded-full bg-gray-200 dark:bg-gray-700"></div>
            ...
            <div class="animate-pulse">
                <div class="mb-2 h-4 w-20 rounded bg-gray-200 dark:bg-gray-700"></div>
                <div class="space-y-3">
        ...
        <div v-else-if="!hasActivities" class="py-8 text-center">
            <ClockIcon class="mx-auto size-12 text-gray-300 dark:text-gray-600" />
                        ...
                        >
                            ...
                            >
                                <component
                                    ...
                                    :class="['size-4', getColorClasses(item.color).icon]"
                                />
                            </div>
                            ...
                            <div class="min-w-0 flex-1 pb-2">
                                <div class="flex items-baseline justify-between gap-2">
                                    <p class="text-sm text-gray-900 dark:text-white">
                                        <span v-if="item.user" class="font-medium">{{ item.user.name }}</span>
                                        <span v-else class="font-medium text-gray-500">System</span>
                                        <span class="ml-1 text-gray-600 dark:text-gray-300">{{ item.description }}</span>
                                    </p>
                                ...
                                <!-- Changes -->
                                <div v-if="item.changes" class="mt-2 rounded-md bg-gray-50 p-2 dark:bg-gray-700/50">
                                    <div
                                    ...
                                    >
                                        ...
                                        <span class="ml-1 text-red-600 line-through dark:text-red-400">{{ change.old }}</span>
