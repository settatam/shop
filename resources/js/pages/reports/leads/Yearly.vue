...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Yearly', href: '/reports/leads/yearly' },
];
...
const kitsRequestedData = computed(() =>
    props.yearlyData.map((row) => row.kits_requested),
);
const kitsReceivedData = computed(() =>
    props.yearlyData.map((row) => row.kits_received),
);
const offersAcceptedData = computed(() =>
    props.yearlyData.map((row) => row.offers_accepted),
);
...
const requestedTrend = computed(() => {
    ...
    const current =
        props.yearlyData[props.yearlyData.length - 1]?.kits_requested || 0;
    const previous =
        props.yearlyData[props.yearlyData.length - 2]?.kits_requested || 0;
...
function viewYear(row: YearRow): void {
    router.visit(
...
<template>
    <Head title="Leads Report (Year over Year)" />
