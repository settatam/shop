...
// Chart data
const chartLabels = computed(() => props.yearlyData.map(row => row.period));
const addedData = computed(() => props.yearlyData.map(row => row.cost_added));
const removedData = computed(() => props.yearlyData.map(row => row.cost_removed));
const netData = computed(() => props.yearlyData.map(row => row.net_cost));
...
// Trends (compare last year vs previous year)
const addedTrend = computed(() => {
    if (props.yearlyData.length < 2) return 0;
    const current = props.yearlyData[props.yearlyData.length - 1]?.cost_added || 0;
    const previous = props.yearlyData[props.yearlyData.length - 2]?.cost_added || 0;
...
const netTrend = computed(() => {
    if (props.yearlyData.length < 2) return 0;
    const current = props.yearlyData[props.yearlyData.length - 1]?.net_cost || 0;
    const previous = props.yearlyData[props.yearlyData.length - 2]?.net_cost || 0;
    if (previous === 0) return current > 0 ? 100 : 0;
    return ((current - previous) / Math.abs(previous)) * 100;
});
...
<template>
    <Head title="Inventory Report - Year over Year" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                />
                <StatCard
                    title="Avg Yearly Added"
                    :value="formatCurrency(avgYearlyAdded)"
                />
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div class="overflow-hidden rounded-lg bg-white shadow ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/10">
                    <div class="border-b border-gray-200 px-4 py-4 dark:border-gray-700">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Yearly Cost Activity</h3>
                    </div>
                    <div class="p-4">
                        <AreaChart
                ...
                <div class="overflow-hidden rounded-lg bg-white shadow ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/10">
                    <div class="border-b border-gray-200 px-4 py-4 dark:border-gray-700">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Yearly Items Volume</h3>
                    </div>
                    <div class="p-4">
                        <BarChart
                            v-if="yearlyData.length > 0"
            ...
            <!-- Data Table -->
            <ReportTable title="Yearly Inventory Data" :export-url="exportUrl" :email-url="emailUrl">
