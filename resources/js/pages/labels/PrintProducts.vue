...
const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { title: 'Print product labels', href: '#' },
];
...
async function generateZpl() {
    if (!selectedTemplateId.value || props.selectedVariantIds.length === 0) return;
    ...
    try {
        const response = await fetch('/print-labels/products/zpl', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                variant_ids: props.selectedVariantIds,
...
function printToBrowser() {
    ...
    // Create a simple preview window for browser printing
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        <Head title="Print product labels" />

        <div class="mx-auto max-w-4xl space-y-6">
            <HeadingSmall
                title="Print product labels"
                :description="`Print labels for ${selectedVariantIds.length} selected product${selectedVariantIds.length === 1 ? '' : 's'}`"
            />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                            You need to create a product label template first.
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">
                                Total labels: {{ selectedVariantIds.length * quantity }}
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                            <thead class="bg-gray-50 dark:bg-white/5">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Barcode</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                                <tr v-for="(v, index) in variants" :key="index">
                                    <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ v.product.title }}</td>
                                    <td class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{{ v.variant.sku || '-' }}</td>
                                    <td class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{{ v.variant.barcode || '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
